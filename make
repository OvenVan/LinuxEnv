    ...
    do
        ...
        ngx_obj=`echo $ngx_obj \
            | sed -e "s#^\(.*\.\)cpp\\$#$ngx_objs_dir\1$ngx_objext#g" \
                  ...
                  -e "s#^\(.*\.\)S\\$#$ngx_objs_dir\1$ngx_objext#g"`
        ngx_src=`echo $ngx_src | sed -e "s/\//$ngx_regex_dirsep/g"`
echo "OVEN:After sed: ngx_src is $ngx_src"
		ext=`echo ${ngx_src:(-3)}`
echo "OVEN: ext is $ext"
		ngx_cxx=$ngx_cc
		if [ $ext = "cpp" ]; then
			ngx_cxx="$ngx_cc \$(CXXFLAGS)"
		fi
echo "OVEN: ngx_cc  is $ngx_cc"
echo "OVEN: ngx_cxx is $ngx_cxx"
        cat << END                                            >> $NGX_MAKEFILE

$ngx_obj:	\$(ADDON_DEPS)$ngx_cont$ngx_src
	$ngx_cxx$ngx_tab$ngx_objout$ngx_obj$ngx_tab$ngx_src$NGX_AUX
echo "OVEN: ngx_obj is $ngx_obj"
...
END
    ...
    do
		...
		#ext=`echo ${ngx_source}|cut -d . -f 2`
		ext=`echo ${ngx_source:(-3)}`
		ngx_cxx=$ngx_cc
		if [ $ext = "cpp" ]; then
			ngx_cxx="$ngx_cc \$(CXXFLAGS)"
		fi
